...
DATPathFormat::DATPathFormat(void) :
	Format(true)
{
...
void							DATPathFormat::unload(void)
{
}
...
void				DATPathFormat::unserializeDefault(void)
{
	...
	DATEntry_Paths_Default_Header *pHeader1 = pDataReader->readStruct<DATEntry_Paths_Default_Header>();
	...
	DATEntry_Paths_Default_PathNode *pPathNodes = new DATEntry_Paths_Default_PathNode[m_header.m_uiPathNodeCount];
	DATEntry_Paths_Default_NaviNode *pNaviNodes = new DATEntry_Paths_Default_NaviNode[m_header.m_uiNaviNodeCount];
	DATEntry_Paths_Default_Link *pLinks = new DATEntry_Paths_Default_Link[m_header.m_uiLinkCount];
	DATEntry_Paths_Default_NaviLink *pNaviLinks = new DATEntry_Paths_Default_NaviLink[m_header.m_uiLinkCount];
	DATEntry_Paths_Default_LinkLength *pLinkLengths = new DATEntry_Paths_Default_LinkLength[m_header.m_uiLinkCount];
	...
	{
		DATEntry_Paths_Default_PathNode &pPathNode_Default = *(pPathNodes + i);
		DATEntry_Paths_General_PathNode &pPathNode_General = m_vecPathNodes[i];
	...
	{
		DATEntry_Paths_Default_NaviNode &pNaviNode_Default = *(pNaviNodes + i);
		DATEntry_Paths_General_NaviNode &pNaviNode_General = m_vecNaviNodes[i];
	...
	{
		DATEntry_Paths_Default_Link			&pLink_Default = *(pLinks + i);
		DATEntry_Paths_Default_NaviLink		&pNaviLink_Default = *(pNaviLinks + i);
		DATEntry_Paths_Default_LinkLength		&pLinkLength_Default = *(pLinkLengths + i);
		DATEntry_Paths_General_Link			&pLink_General = m_vecLinks[i];
...
void				DATPathFormat::unserializeFastman92(void)
{
	...
	DATEntry_Paths_Fastman92_PathNode_extended *pPathNodes = new DATEntry_Paths_Fastman92_PathNode_extended[m_header.m_uiPathNodeCount];
	DATEntry_Paths_Fastman92_NaviNode_extended *pNaviNodes = new DATEntry_Paths_Fastman92_NaviNode_extended[m_header.m_uiNaviNodeCount];
	DATEntry_Paths_Fastman92_Link *pLinks = new DATEntry_Paths_Fastman92_Link[m_header.m_uiLinkCount];
	DATEntry_Paths_Fastman92_NaviLink *pNaviLinks = new DATEntry_Paths_Fastman92_NaviLink[m_header.m_uiLinkCount];
	DATEntry_Paths_Fastman92_LinkLength *pLinkLengths = new DATEntry_Paths_Fastman92_LinkLength[m_header.m_uiLinkCount];
	DATEntry_Paths_Fastman92_PathIntersectionFlags *pPathIntersectionFlags = new DATEntry_Paths_Fastman92_PathIntersectionFlags[m_header.m_uiLinkCount];
	...
	*/
	...
	for (uint32 i = 0, j = m_header.m_uiPathNodeCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_PathNode_extended *pPathNode_Fastman92 = pPathNodes + i;
	...
	for (uint32 i = 0, j = m_header.m_uiNaviNodeCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_NaviNode_extended *pNaviNode_Fastman92 = pNaviNodes + i;
	...
	for (uint32 i = 0, j = m_header.m_uiLinkCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_Link *pLink_Fastman92 = pLinks + i;
	...
	for (uint32 i = 0, j = m_header.m_uiLinkCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_NaviLink *pNaviLink_Fastman92 = pNaviLinks + i;
		pNaviLink_Fastman92->m_uiData = pDataReader->readUint32();
	}

	for (uint32 i = 0, j = m_header.m_uiLinkCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_LinkLength *pLinkLength_Fastman92 = pLinkLengths + i;
		pLinkLength_Fastman92->m_ucLength = pDataReader->readUint8();
	}

	for (uint32 i = 0, j = m_header.m_uiLinkCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_PathIntersectionFlags *pPathIntersectionFlags_Fastman92 = pPathIntersectionFlags + i;
...
void							DATPathFormat::serializeDefault(void)
{
	...
	DATEntry_Paths_Default_Header header;
	...
	for (uint32 i = 0, j = m_header.m_uiPathNodeCount; i < j; i++)
	{
		...
		pPathNode_Default.m_usLinkId = (uint16) pPathNode_General.m_wLinkId;
		pPathNode_Default.m_usNodeId = (uint16) pPathNode_General.m_wNodeId;
		pPathNode_Default.m_usAreaId = (uint16) pPathNode_General.m_wAreaId;
	...
	for (uint32 i = 0, j = m_header.m_uiNaviNodeCount; i < j; i++)
	{
		DATEntry_Paths_Default_NaviNode &pNaviNode_Default = *(pNaviNodes + i);
		DATEntry_Paths_General_NaviNode &pNaviNode_General = m_vecNaviNodes[i];
		pNaviNode_Default.m_iPosition[0] = (int16)pNaviNode_General.m_iPosition[0];
		pNaviNode_Default.m_iPosition[1] = (int16)pNaviNode_General.m_iPosition[1];
		pNaviNode_Default.m_ucDirection[0] = (int16)pNaviNode_General.m_ucDirection[0];
		pNaviNode_Default.m_ucDirection[1] = (int16)pNaviNode_General.m_ucDirection[1];
...
void							DATPathFormat::serializeFastman92(void)
{
	...
	for (uint32 i = 0, j = m_header.m_uiPathNodeCount; i < j; i++)
	{
		...
		pPathNode_Fastman92.m_pPrev = 0;
		pPathNode_Fastman92.m_ppNext = 0;
		pPathNode_Fastman92.m_posn.x = 0;
		pPathNode_Fastman92.m_posn.y = 0;
		pPathNode_Fastman92.m_posn.z = 0;
		...
		pPathNode_Fastman92.m_wNodeId = (int16) pPathNode_General.m_wNodeId;
		pPathNode_Fastman92.m_wAreaId = (int16) pPathNode_General.m_wAreaId;
	...
	for (uint32 i = 0, j = m_header.m_uiNaviNodeCount; i < j; i++)
	{
		DATEntry_Paths_Fastman92_NaviNode_extended &pNaviNode_Fastman92 = *(pNaviNodes + i);
		DATEntry_Paths_General_NaviNode &pNaviNode_General = m_vecNaviNodes[i];
		pNaviNode_Fastman92.posX = 0;
		pNaviNode_Fastman92.posY = 0;
...
void					DATPathFormat::applyOffsetToPositions(Vec3f& vecPositionOffset)
{
	for (auto& pathNode : m_vecPathNodes)
	{
		...
		x += vecPositionOffset.x;
		y += vecPositionOffset.y;
		z += vecPositionOffset.z;
...
void					DATPathFormat::addPathNode(DATEntry_Paths_General_PathNode pathNode)
{
	...
	DATEntry_Paths_General_PathNode pathNode2 = pathNode;
	pathNode2.m_wAreaId = m_uiFileIndex;
	pathNode2.m_wNodeId = m_vecPathNodes.size();
	m_vecPathNodes.push_back(pathNode2);
}

void					DATPathFormat::addNaviNode(DATEntry_Paths_General_NaviNode naviNode)
{
	m_header.m_uiNaviNodeCount++;
	DATEntry_Paths_General_NaviNode naviNode2 = naviNode;
	//naviNode2.m_usAreaId = m_uiFileIndex;
	//naviNode2.m_usNodeId = m_vecNaviNodes.size();
	m_vecNaviNodes.push_back(naviNode2);
}

void					DATPathFormat::addLink(DATEntry_Paths_General_Link link)
{
	m_header.m_uiLinkCount++;
	DATEntry_Paths_General_Link link2 = link;
