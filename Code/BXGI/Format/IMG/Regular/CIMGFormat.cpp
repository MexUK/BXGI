...
void		CIMGFormat::unserializeVersion1(void)
{
	...
	for (uint64 i = 0; i < m_uiEntryCount; i++)
	{
		...
		CEventManager::get()->triggerEvent(EVENT_onParseIMGEntry, this);
	}
...
void					CIMGFormat::serializeVersion1(void)
{
	...
	for (auto pIMGEntry : getEntries())
	{
		...
		CEventManager::get()->triggerEvent(EVENT_onStoreIMGEntry, this);
	}
...
void					CIMGFormat::addEntry(CIMGEntry *pIMGEntry)
{
	...
	CEventManager::get()->triggerEvent(EVENT_onAddIMGEntry, pIMGEntry);
	...
	{
		CEventManager::get()->triggerEvent(EVENT_onAddIMGEntryExtension, pIMGEntry);
	}
}
...
void								CIMGFormat::removeEntry(CIMGEntry *pIMGEntry)
{
	...
	{
		CEventManager::get()->triggerEvent(EVENT_onRemoveIMGEntryExtension, pIMGEntry);
	}

	CEventManager::get()->triggerEvent(EVENT_onRemoveIMGEntry, pIMGEntry);
...
uint32						CIMGFormat::replaceEntries(vector<string>& vecPaths, vector<string>& vecReplacedEntryNames, vector<CIMGEntry*>& vecReplacedEntries)
{
	...
	for (auto strPath : vecPaths)
	{
		...
		if (!pIMGEntry)
		{
			CEventManager::get()->triggerEvent(EVENT_onTaskProgress, this);
			continue;
		}
