...
#include "WDRBlock_VertexData.h"
#include "WDRBlock_IndexData.h"
...
void						WDRFormat::_serialize(void)
{
	...
	pDataWriter->writeUint32(WDRManager::getPackedOffset(0x90)); // pointer ModelCollection
	...
	pDataWriter->writeUint32(WDRManager::getPackedOffset(uiSeek + 16)); // pointer to Model pointer
	...
	pDataWriter->writeUint32(WDRManager::getPackedOffset(uiSeek + 4)); // Model pointer
	uiSeek += 4;
	for (uint32 i = 0, j = m_vecModels.size(); i < j; i++)
	{
		pDataWriter->writeUint32(WDRManager::getPackedOffset(uiModelArrayStartPosition + (i * 32))); // 32 = sizeof(Model)
		uiSeek += 4;
	}
	...
	for (auto pWDRBlock_Model : m_vecModels)
	{
		...
		pDataWriter->writeUint32(WDRManager::getPackedOffset(uiGeometryPointerCollectionArrayStartPosition + (uiModelIndex * 8))); // 8 byte offset PointerCollection<Geometrys>
	...
	for (auto pWDRBlock_Model : m_vecModels)
	{
		...
		pDataWriter->writeUint32(WDRManager::getPackedOffset(uiPointersStartPosition + uiOffset2)); // offset to pointers
	...
	for (auto pWDRBlock_Model : m_vecModels)
	{
		...
		{
			pDataWriter->writeUint32(WDRManager::getPackedOffset(uiGeometryArrayStartPosition + uiOffset2)); // offset to Geometry
			uiOffset2 += 80 + 64 + 48;
		}
	}
	...
	for (auto pWDRBlock_Model : m_vecModels)
	{
		...
		for (auto pWDRBlock_Geometry : pWDRBlock_Model->m_vecGeometries)
		{
			...
			pDataWriter->writeUint32(WDRManager::getPackedOffset(uiVertexBufferStartPosition)); // offset VertexBuffer
			pDataWriter->writeUint32(0); // unknown 3
			pDataWriter->writeUint32(0); // unknown 4
			pDataWriter->writeUint32(0); // unknown 5
			pDataWriter->writeUint32(WDRManager::getPackedOffset(uiIndexBufferStartPosition)); // offset IndexBuffer
			...
			pDataWriter->writeUint16((uint16)(pWDRBlock_Geometry->m_vecVertices.size())); // vertex count
			pDataWriter->writeUint16(0); // unknown 1
			pDataWriter->writeUint32(WDRManager::getPackedDataOffset(uiVertexDataArrayStartPosition + uiOffset2)); // data offset to vertex data
			...
			pDataWriter->writeUint32(pWDRBlock_Geometry->m_vecIndices.size()); // index count
			pDataWriter->writeUint32(WDRManager::getPackedDataOffset(uiVertexDataArrayStartPosition + uiOffset2 + (true ? 36 : 52))); // data offset to index data
			pDataWriter->writeUint32(0); // unknown 1
			for (uint32 i = 0; i < 8; i++)
			{
				pDataWriter->writeUint32(0xCDCDCDCD); // 32 bytes padding
...
WDRBlock_Geometry*			WDRFormat::convertIntermediateGeometryToWDRGeometry(IntermediateGeometry *pGeneralGeometry)
{
	...
	for (IntermediateVertexData& vertexData : pGeneralGeometry->getVertices())
	{
		WDRBlock_VertexData *pWDRBlock_VertexData = new WDRBlock_VertexData;
	...
	for (IntermediateIndexData& indexData : pGeneralGeometry->getTriangles())
	{
		WDRBlock_IndexData *pWDRBlock_IndexData = new WDRBlock_IndexData;
